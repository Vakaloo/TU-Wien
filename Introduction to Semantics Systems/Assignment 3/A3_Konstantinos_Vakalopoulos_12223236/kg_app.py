...
def LoadOntology():
    your_ontology = input("Load your ontology from the URI or from a file with the destination included: ")
    g = Graph()
    g.parse(your_ontology)
    return g

def ActivateReasoner(g):
    print ("1. RDFS Reasoner")
    print ("2. OWL2-RL + RDFS")
    reasoner_choice = int(input("Choose the reasoner (1 or 2): "))

    if reasoner_choice == 1:
        owlrl.DeductiveClosure(owlrl.RDFS_Semantics).expand(g)
    elif reasoner_choice == 2:
...
while menu_chosen:
    ...
    if menu_chosen == 1:
        ...
        try:
            ExecuteSelectQuery(graph)
        except:
            print("There has been an error executing the select query. Please try again!")
    elif menu_chosen == 9:
        try:
            ExecuteConstructQuery(graph)
            print("The query has been created at the given destination\n")
        except:
