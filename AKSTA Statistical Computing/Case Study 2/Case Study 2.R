...
df_vars %>% count(Developed...Developing.Countries)

##############################################################################

##############################Question 7######################################

df_vars %>% count(Region.Name)
...
df_vars_removed_NA <- df_vars %>% filter(!is.na(Median_Age))
df_vars_removed_NA <- df_vars_removed_NA %>% filter(!is.na(youth_unempl_rate))
...
average_region <- df_vars_removed_NA %>% 
  group_by(Region.Name) %>%
...
for (region in average_region$Region.Name){
  ...
  filtered_data_per_region <- df_vars_removed_NA %>% filter(Region.Name==region)
  filtered_average <- average_region %>% filter(Region.Name==region)
  
  
  temporary <- filtered_data_per_region %>% 
    mutate(above_average_median_age = case_when(
