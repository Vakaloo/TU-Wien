...
class Recommender_MF(BaseEstimator, RegressorMixin):
    ...
    def fit_init(self, X, y, n_users, n_items):
        ...
        self.P_ =  np.random.normal(self.rnd_mean, self.rnd_std,size=(n_users, self.k))
        self.Q_ =  np.random.normal(self.rnd_mean, self.rnd_std, size=(n_items, self.k))
        self.bu_ =  np.random.normal(self.rnd_mean, self.rnd_std, size=(n_users,)) 
        self.bi_ =  np.random.normal(self.rnd_mean, self.rnd_std, size=(n_items,)) 
        ...
        for epoch in range(self.n_epochs):
            ...
            for index in range(self.y_.shape[0]):
                ...
                if self.w_average and self.w_biases:
                    prediction = self.mu_+self.bu_[u]+self.bi_[i]+np.dot(self.P_[u,], self.Q_[i,])
                elif self.w_average and not self.w_biases:
                    prediction = self.mu_+np.dot(self.P_[u,], self.Q_[i,])
                elif not self.w_average and self.w_biases:
                    prediction = self.bu_[u]+self.bi_[i]+np.dot(self.P_[u,], self.Q_[i,])
                else:
                    prediction = np.dot(self.P_[u,], self.Q_[i,])

                err = r_ui-prediction

                delta_Pu = err*self.Q_[i,] - self.lam*self.P_[u,]
                delta_Qi = err*self.P_[u,] - self.lam*self.Q_[i,]

                self.P_[u,] = self.P_[u,] + self.eta*delta_Pu
                self.Q_[i,] = self.Q_[i,] + self.eta*delta_Qi
